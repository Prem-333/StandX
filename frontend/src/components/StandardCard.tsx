...
export function StandardCard({ item, index, reportId, apiKey }: StandardCardProps) {
  ...
  const scoreColor   = scorePercent >= 80 ? '#069669' : scorePercent >= 55 ? '#1d4ed8' : '#d97706';

  return (
    <article
      className="card card-hover flex flex-col animate-slide-up"
      style={{ animationDelay: `${index * 80}ms`, animationFillMode: 'both', borderLeft: '3px solid #1d4ed8' }}
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-3 mb-4">
          <div className="flex flex-wrap items-center gap-2.5">
            <span
              ...
              style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1d4ed8 100%)', boxShadow: '0 2px 8px rgba(30,58,95,0.2)' }}
            >
