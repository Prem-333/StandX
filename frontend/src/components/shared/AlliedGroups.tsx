...
export function AlliedGroups({ groups }: { groups: Record<string, Allied[]> }) {
  ...
  if (!populated.length) {
    return <p className="text-[12px] text-on-surface-variant italic">No evidenced allied standards in this KB snapshot.</p>;
  }
  return (
      ...
      {populated.map(([relation, rows]) => (
        <details key={relation} className="group allied-group">
          <summary className="flex items-center gap-2.5 py-1.5 px-2 -ml-2 rounded-lg text-[12px] font-semibold text-on-surface-variant cursor-pointer list-none hover:bg-surface-container-low transition-colors">
            <Layers3 size={12} className="flex-shrink-0 text-secondary opacity-70" />
            <span>{relationshipNames[relation] || titleCase(relation)}</span>
            <span className="chip-navy px-1.5 py-0.5 rounded text-[10px] font-bold ml-1">{rows.length}</span>
            <ChevronDown size={12} className="ml-auto text-on-surface-variant group-open:rotate-180 transition-transform duration-200" />
          </summary>
            ...
            {rows.map((row) => (
              <div key={row.record_id} className="relative allied-item">
                ...
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <span className="text-[13px] font-bold text-secondary">{row.is_number}</span>
                  <Provenance source={row.source} />
                </div>
                <p className="text-[12px] text-on-surface-variant leading-relaxed max-w-2xl">{row.title}</p>
