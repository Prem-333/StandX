    ...
    <div className="space-y-3">
      {uniqueRules.map((rule, index) => (
        <details className="group rule" key={`${rule.id}-${index}`}>
          <summary className="flex items-center gap-3 list-none cursor-pointer p-2 -ml-2 rounded-xl hover:bg-white/5 transition-colors">
            <span
            ...
            >
              <ShieldCheck size={12} />
              {rule.scheme === 'HALLMARKING' ? 'Hallmark' : rule.scheme} ·{' '}
              {titleCase(rule.requirement)}
            </span>
            <span className="text-[11px] text-[var(--clr-text-muted)]">
              Verified {rule.as_verified_on}
            </span>
          ...
          <div className="mt-3 ml-2 pl-4 border-l-2 border-[var(--clr-teal)]/30 text-[13px] leading-relaxed text-[var(--clr-text-dim)] space-y-3">
            <div>
            ...
            <p className="text-[var(--clr-text-dim)]">
              {rule.stale
            ...
            <div className="space-y-1.5">
              <p>{rule.notice}</p>
              {rule.scope_note && <p className="text-[var(--clr-text-muted)] italic">{rule.scope_note}</p>}
            </div>
            <p className="text-[11px] text-[var(--clr-text-muted)] bg-[var(--clr-bg)] inline-block px-2.5 py-1 rounded-lg border border-[var(--clr-border)]">
              Triggered by:{' '}
              <span className="font-bold text-[var(--clr-text)]">
                {String(rule.trigger.value || rule.product_category)}
              </span>
              {rule.trigger.record_id ? ` · KB record ${String(rule.trigger.record_id)}` : ''}
            </p>
            {(rule.missing_conditions?.length ?? 0) > 0 && (
              <ul className="list-disc list-inside space-y-1 text-[var(--clr-text-dim)]">
                {rule.missing_conditions?.map((c) => (
                  <li key={c.description}>{c.description}</li>
                ))}
              </ul>
            )}
            {rule.evidence.map((e, i) => (
              <div key={i} className="bg-[var(--clr-bg-2)] p-3 rounded-xl border border-[var(--clr-border)]">
                <a
                  href={publicUrl(e.url)}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[var(--clr-teal)] hover:text-[var(--clr-teal-dim)] font-semibold inline-flex items-center gap-1 text-xs"
                >
                  {e.locator} <ChevronRight size={11} />
                </a>
