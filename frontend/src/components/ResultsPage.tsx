    ...
    <div className="animate-slide-up">
      ...
      {report.synthetic_enabled && (
        <div className="flex items-center gap-3 text-[13px] mb-5 status-banner-warn px-4 py-3">
          <span className="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-black chip-amber">
            DEMO WORKSPACE
          </span>
          <span className="text-amber-700">
            Mock / synthetic results enabled. Fixtures have no legal effect.
          </span>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] mt-2.5 text-on-surface-variant font-bold uppercase tracking-widest opacity-70">
                All candidates require officer review; a score does not
          ...
          {report.primary_standards.length ? (
            ...
            <section className="card p-12 text-center flex flex-col items-center">
              ...
              <h2 className="text-headline-md text-on-surface font-bold mb-1.5">
                No candidates to show
              </h2>
              <p className="text-[14px] text-on-surface-variant max-w-sm mx-auto mb-6 leading-relaxed">
                The local knowledge base did not produce a match. Try a product
                name, material, or a known IS number.
              </p>
              <button
        ...
        <aside className="flex flex-col gap-4 sticky top-20">
          ...
          <section className="card p-4">
            <h2 className="flex items-center gap-2 text-[14px] font-bold text-on-surface mb-3">
              <FileText size={13} className="text-secondary" /> Your
              specification
            </h2>
            <div className="p-3 rounded-xl border border-surface-container bg-surface-container-low">
              <p
                className="original-text text-[13px] leading-relaxed text-on-surface-variant max-h-[160px] overflow-y-auto whitespace-pre-wrap break-words"
                lang={
                  report.normalization.detected_language === "hi"
                    ? "hi"
                    : undefined
                }
              >
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest">
              ...
              {report.normalization.detected_language} ·{" "}
              {titleCase(report.normalization.status)}
            </div>
              ...
              <>
                ...
                <div className="p-3 rounded-xl border border-surface-container bg-surface-container-low">
                  <p className="text-[12px] text-on-surface-variant whitespace-pre-wrap">
                    {report.normalization.normalized_text ||
            ...
            {report.normalization.notices.length > 0 && (
              <div className="mt-3 space-y-2">
                {report.normalization.notices.map((notice, i) => (
                  <p
                    key={i}
                    className="status-banner-warn text-amber-700 text-[12px] p-3 leading-relaxed flex gap-1.5"
                  >
          ...
          <section className="card p-4">
            <h2 className="flex items-center gap-2 text-[14px] font-bold text-on-surface mb-1">
              <ShieldCheck size={13} className="text-secondary" /> Category
              rules
            </h2>
            <p className="text-[12px] text-on-surface-variant mb-3 leading-relaxed">
              Dated mappings from the shortlisted records and any category you
              supplied.
            </p>
            ...
            (report.extraction.manual_review_clauses.length > 0 ||
              ...
              <section className="card p-4 status-banner-warn border-amber-300">
                ...
                <p className="text-[13px] text-on-surface-variant leading-relaxed">
                  Some clauses could not be resolved or exceeded the extraction
                  limit.
                </p>
          ...
          <section className="card p-4">
            <h2 className="flex items-center gap-2 text-[14px] font-bold text-on-surface mb-1">
              <Link2 size={13} className="text-secondary" /> Evidence trail
              saved
            </h2>
            <p className="text-[12px] text-on-surface-variant leading-relaxed mb-3">
              Input, scores, source records, and rule snapshots are recorded for
              review.
            </p>
