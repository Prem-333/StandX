...
export function Header({
  ...
  return (
    <>
      <header className="fixed top-0 left-0 md:left-64 right-0 h-14 glass-panel border-b border-surface-container/40 z-40 flex items-center justify-between px-5">
        {/* Breadcrumb */}
        <div className="flex items-center gap-1.5 min-w-0">
          <span className="text-[12px] font-medium text-outline tracking-wide uppercase hidden sm:block">
            StandX
          </span>
          <span className="text-outline text-[12px] hidden sm:block">/</span>
          <span className="text-[14px] font-semibold text-on-surface tracking-tight truncate">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <button
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-surface-container-low text-on-surface text-[12px] font-medium hover:bg-surface-container transition-colors focus-ring"
            onClick={() => setConnection(!connection)}
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${connected ? "bg-on-tertiary-container" : "bg-error"}`}
            ></span>
          ...
          {/* Search bar */}
          <button
            aria-label="Search local standards"
            onClick={() => setScreen("directory")}
            className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-lg bg-surface-container-low text-on-surface-variant hover:bg-surface-container transition-colors focus-ring"
          >
            <Search size={15} />
            <span className="text-[14px] hidden md:inline text-on-surface-variant">
              Search local standards
            </span>
            <kbd className="text-[11px] px-1.5 py-0.5 rounded bg-surface-container-lowest text-outline shadow-sm uppercase font-mono hidden lg:block">
              Ctrl / ⌘ K
            </kbd>
          </button>
